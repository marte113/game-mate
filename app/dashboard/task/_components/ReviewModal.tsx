...
export default function ReviewModal() {
  ...
  const getRatingText = (currentRating: number | null): string => {
    ...
    const closest = StarRatingTexts.reduce((prev, curr) =>
      Math.abs(curr.rating - currentRating) < Math.abs(prev.rating - currentRating) ? curr : prev,
    )
    return closest.text
  }
  ...
  const renderStarIcon = (index: number, ratingValue: number | null) => {
    ...
    if (value >= index + 1) {
      ...
      // 반 별
      return <StarHalf className="w-full h-full text-amber-400 fill-amber-400" />
    } else {
      // 빈 별
            ...
            <div className="flex justify-center items-center space-x-1 mb-1">
              {[0, 1, 2, 3, 4].map((index) => (
                <div key={index} className="relative w-5 h-5">
                  ...
                  {/* 크기 조정 */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    {renderStarIcon(index, rating)} {/* 클릭된 rating 값으로 표시 */}
                  </div>
                </div>
              ))}
            </div>
        ...
        return (
          <>
            ...
            <div className="border p-4 rounded-lg bg-base-200">
              <p className="text-sm font-medium text-base-content/70 mb-3">작성된 후기 미리보기:</p>
    ...
    <div className="modal modal-open">
      <div className="modal-box relative">
        ...
        <div className="modal-action mt-6">
          ...
          <button
            className={`btn btn-primary ${phase === 1 && rating === null ? "btn-disabled" : ""}`}
