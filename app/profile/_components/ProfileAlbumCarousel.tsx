...
export default function ProfileAlbumCarousel({ userId: _userId, profileNickname }: ProfileAlbumCarouselProps) {
  // 앨범 이미지 데이터 Fetch (중앙 훅 사용)
  const { data, isLoading, error } = useAlbumImagesQuery()

  // 로딩 또는 이미지 없는 경우 렌더링 안 함
  if (isLoading || error || !data?.images || data.images.every((v: any) => v == null)) {
  ...
  const visible = formatted.filter((i): i is { id: string; url: string; isThumbnail: boolean } => Boolean(i))
